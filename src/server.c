...
char buf[BUFSIZ],sbuf[BUFSIZ];
struct sockaddr_in myaddr; 	/* address of this service */
struct sockaddr_in client_addr; /* address of client    */
char cst[255];
//--------------------------------------------------------------------------------------------

char Rx_Tcpbuffer[BUFSIZ];
...
{	
		if(tcp_send_flag==1 && client_connect_flag)
		{
			...
			{		
				printf("Send to client, Message:%s , len = %d\n",s,strlen(s));
			}
			tcp_send_flag = 0;
			memset(s,0,strlen(s));
		}
...
void * Task_tcp_server_listen(void* arg)
{
	...
	while (1)
	{
		...
		else
		{
			...
			{
				memset(Rx_Tcpbuffer,0x00,BUFSIZ);
				TaskTcpRxLenth_Flag = recv(recfd, Rx_Tcpbuffer, BUFSIZ,0);
