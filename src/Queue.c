...
void* Task_Queue(void* arg)
{
	...
	if((Queue_TX_Head_Ptr!=Queue_TX_Tail_Ptr) && client_connect_flag) 
	{
...
{   
	 ...
	 while((Queue_RX_Tail_Ptr < QUEUE_RX_MAX_VALUE) )
	 {
		 ...
		 printf("[RX_DB] New Data !!\n");
	 	 printf(YELLOW"[RX_DB]:%s, Queue RX Data read pointer=%d \n"NONE,Data,Queue_RX_Tail_Ptr);
...
{   
	 ...
	 while((Queue_TX_Tail_Ptr < QUEUE_TX_MAX_VALUE) )
	 {
	     if(Queue_TX_Table[Queue_TX_Tail_Ptr].Active == 1)
	     {
	         ...
	         memcpy(Data,Queue_TX_Table[Queue_TX_Tail_Ptr].data,strlen(Queue_TX_Table[Queue_TX_Tail_Ptr].data));

	         *rlen=Queue_TX_Table[Queue_TX_Head_Ptr].len;
		 printf("[TX_DB] New Data !!\n");
	 	 printf(BRON"[TX_DB]:%s, Queue TX data read pointer=%d \n"NONE,Data,Queue_TX_Tail_Ptr);
